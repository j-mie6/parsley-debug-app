...
object Tauri {
     ...
     */
    def invoke(cmd: Command, args: cmd.In): EventStream[Either[DillError, cmd.Out]] = cmd.invoke(args)
     ...
     */
    def listen(event: Event): (EventStream[Either[DillError, event.Out]], Future[() => Unit]) = event.listen()
